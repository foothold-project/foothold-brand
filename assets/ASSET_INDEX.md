...
**Status:** v1.0.2 geometry, with raster delivery expanded 2026-08-11
...
- `FOOTHOLD_ASSET_PACK_V1.zip`: combined SVG + PNG + JPG package.
- `FOOTHOLD_OSMU_V1_2.zip`: current approved medium-specific OSMU SVG + PNG + JPG package.
...
- `raster/v1/jpeg/logo/*.jpg`: approved core logo companions on brand paper or engineering dark; favicon intentionally excluded.
- `raster/v1/jpeg/{retired,legacy,provisional}/**/*.jpg`: lifecycle-separated background-filled companions for historical and review use.

Every PNG and JPG is generated by `scripts/generate-raster-assets.mjs`. Exact PNG metadata lives in `raster/v1/manifest.json`; JPG canvas, background, lifecycle, source hash, and output hash live in `raster/v1/jpeg/manifest.json`. Never edit a raster derivative and use it to recreate the canonical SVG.

## Current approved OSMU v1.2

The six approved compositions live in `osmu/v1.2/`. Their transparent PNG and background-filled JPG derivatives live in `raster/v1.2/osmu/`. Approval provenance and hashes are recorded in `osmu/v1.2/APPROVAL.json` and `osmu/v1.2/manifest.json`.

Do not select same-named files from `exports/v1` or `raster/v1/{web,github,presentation,poster,social,goods}` for new work. Those paths contain superseded legacy proofs.
